...
export default new Vuex.Store({
    state: {
        ...
        packageDir: '',
        isValidDir: false,
        shouldAutoCheckForUpdates: true,
        launchMode: 'steam',
        closeOnLaunch: true,
        gameVersion: '0.21',
        browserMods: [],
    ...
    actions: {
        async loadSavedState({
            ...
            const shouldCheckForUpdates = localStorage.getItem('shouldCheckForUpdates');
            if (shouldCheckForUpdates !== null) commit('setSetting', {
                key: 'shouldCheckForUpdates',
                value: shouldCheckForUpdates === "true"
            });

            const launchMode = localStorage.getItem('launchMode');
            if (launchMode) commit('setSetting', {
                key: 'launchMode',
                value: launchMode
            });

            const closeOnLaunch = localStorage.getItem('closeOnLaunch');
            if (closeOnLaunch !== null) commit('setSetting', {
                key: 'closeOnLaunch',
                value: closeOnLaunch === "true"
            });
        ...
        async loadMods({
            ...
            const modsWithDisabledFlag = mods.map(mod => ({
                ...mod,
                disabled: state.disabledMods.includes(mod.id)
            }));
            ...
            if (state.shouldCheckForUpdates) {
                for (const mod of modsOrderedByLoadOrder) {
                    await dispatch('checkForUpdates', state.packageDir, mod.id);
                }
            }
        },
        async loadMod({
            ...
            if (state.shouldCheckForUpdates) {
                await dispatch('checkForUpdates', state.packageDir, modToLoad);
            }
        },
        async setModDisabledState({
            state,
            dispatch
        }, {
            id,
            ...
            const mods = state.mods.map(mod => mod.id === id ? {
                ...mod,
                disabled
            } : mod);
            await dispatch('setMods', mods);
        ...
        async checkForUpdates({
            ...
            } else if (mod.origin === 'git') {
                updateAvailable = true;
            } else {
                updateAvailable = await api.mods.checkForUpdates(state.packageDir, mod);
            }
            if (updateAvailable) commit('setMod', {
                ...mod,
                updateAvailable
            });
        },
        async updateAllMods({
            ...
            for (const mod of state.mods) {
                if (mod.updateAvailable) await dispatch('updateMod', state.packageDir, mod);
            }
        },
        async updateMod({
            state,
            ...
            await api.mods.update(state.packageDir, id, browserData); // git has browserData === null
            await dispatch('loadMod', id);
            commit('endUpdateMod');
        },
        async removeMod({
