...
const handlers = {
    [mods.parseFile]: async ({
        ...
        try {
            ...
            console.error(e);
            return {
                error: 'Unable to parse the selected file. Make sure it is a valid userscript or WebExtensions extension manifest.'
            };
        }
    },
    ...
    [mods.parseWeb]: async ({
        ...
        try {
            ...
            if (!isWebOrigin(origin) || (!fromBrowser && !isGreasyForkUrl(origin)))
                return {
                    error: 'Only web scripts from GreasyFork are currently supported.'
                };

            const script = await downloadScript(origin);

            if (!script) return {
                error: 'Unable to retrieve script.'
            };
            ...
            console.error(e);
            return {
                error: 'Unable to parse the selected script.'
            };
        }
    },
    ...
    [mods.cloneGit]: async ({
        ...
        try {
            ...
            const gitScript = await downloadGitRepository(origin, packageDir);
            if (gitScript.error) return {
                error: gitScript.error
            };
    ...
    [mods.browserInstall]: async ({
        ...
        if (data.type === 'script') {
            ...
            const dl = await download(BrowserWindow.getFocusedWindow(), data.download, {
                directory: tempPath
            });
            ...
            const manifest = await handlers[mods.parseFile]({
    ...
    [mods.add]: async ({
        ...
        try {
            ...
            await writeJson(path.join(modPath, 'manifest.json'), manifest, {
                spaces: 2
            });
            ...
            return {
                error: 'Unable to add the selected mod.'
            };
        }
    },
    ...
    [mods.load]: async ({
        ...
        try {
            ...
            console.error(e);
            return {
                error: `Unable to load mod ${id}.`
            };
        }
    },

    [mods.checkForUpdates]: async ({
        packageDir,
		mod
    }) => {
        try {
            console.log("Checking for updates");
            ppJson({packageDir, mod});
            return await getUpdates(mod, packageDir);
        } catch (e) {
            console.error(e);
            return null;
        }
    ...
    [mods.update]: async ({
        ...
        try {
            ...
            } else if (manifest.origin === 'git') {
                return await updateExtensionGit(modPath);
            }
...
const downloadScript = async (url, packageDir) => {
    if (isGreasyForkUrl(url)) {
        return await downloadGreasyForkScript(url);
    }
    if (isGitUrl(url)) {
        return await downloadGitRepository(url, packageDir);
    }
...
const downloadGreasyForkScript = async url => {
    try {
        console.log('Downloading Greasyfork script...', JSON.stringify({
            url
        }, null, 2));
        ...
        const {
            data
        } = await axios.get(`https://greasyfork.org${scriptUrl}`);
        return {
            file: scriptFile,
            content: data
        };
...
const downloadGitRepository = async (url, packageDir, modPath) => {
    try {
        ...
        try {
            ...
            return {
                error: 'Couldn\'t clone the repository. Are you sure it exists?'
            };
        }
        return {
            name: repoName,
...
const getGreasyForkUpdates = async mod => {
    console.log(`Fetching GreasyFork updates for ${mod.id}...`);
    const {
        data
    } = await axios.get(mod.origin);
...
const getGitUpdates = async (mod, packageDir) => {
    console.log(`Fetching git updates for ${mod.id}...`);
    const git = simpleGit(getModPath(packageDir, mod)).env('GIT_TERMINAL_PROMT', '0');
    await git.fetch();

    const currentVersion = mod.version.split(".");
    let latestVersion = currentVersion;
    // we're cheating the version to always be newer
    latestVersion[latestVersion.length - 1] = latestVersion[latestVersion.length - 1] + 1

    let isNewVersion = latestVersion > currentVersion
...
const updateScript = async (modPath, manifestPath, manifest, browserData) => {
    console.log(`Updating script ${manifest.id}...`);
    const url = manifest.origin === 'browser' ? browserData.download : manifest.origin;
    const {
...
const updateExtensionGit = async (modPath, manifestPath, manifest, browserData) => {
	console.log(`Updating ${manifest.id}...`);
    const git = simpleGit(modPath).env('GIT_TERMINAL_PROMPT', '0');
    await git.pull();
...
const updateExtension = async (modPath, manifestPath, manifest) => {
    const tempPath = path.join(app.getPath('temp'), 'M3');
    await ensureDir(tempPath);
    const dl = await download(BrowserWindow.getFocusedWindow(), browserData.download, {
        directory: tempPath
    });
    const extDir = path.join(tempPath, dl.getFilename().split('.')[0]);
    await decompress(dl.savePath, extDir);
    ...
    const tempManifest = await handlers[mods.parseFile]({
  ...
  for (const mod of mods) {
	for (const script of mod.scripts) {
	  ...
	  try {
		...
		console.log("Could not load mod's content", e);
	  }
	}
